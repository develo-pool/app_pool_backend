...
public interface BrandUserRepository extends JpaRepository<BrandUser, Long> {
    ...
    @Query(value = "SELECT * FROM brand_user WHERE pool_user_id IN (SELECT pool_user_id FROM pooluser WHERE user_status = 'BRAND_USER')  ORDER BY brand_user_id DESC limit 3", nativeQuery = true)
